...
let to_value_type name arg_type = match arg_type with
  ...
  | _ -> if String.is_suffix arg_type ~suffix:"*" then sprintf "val_of_ptr((void *)%s)" name else raise (InvalidFunction (sprintf "Could not to_value %s" arg_type))
;;
...
let rec matches_chr chr t = match chr with
  | [] -> false
...
let rec split_inclusive chr = function
  ...
  | (x::xs) -> let (before_split, after_split) = split_inclusive chr xs in x::before_split, after_split
;;
...
let extract_arguments args_str =
  if String.(=) (String.strip args_str) "void" then [] else
  ...
  List.map unprocessed_list ~f:(fun arg_item ->
    ...
    | Some v -> let (l, r) = v in (String.strip (String.concat [l; "*"]), String.strip r)
    | _ -> let (l, r) = String.rsplit2_exn arg_item ~on:' ' in (String.strip l, String.strip r)
  )
...
let process_header_file preamble_file filepath output =
  ...
  let lines = List.map ~f:(fun x -> String.of_char_list x) (full_split [';'; '/'] lines) in
  let rec do_lines = function
    ...
    | (x::xs) -> (
      ...
      | Some fn -> (
        ...
        let generated_fn = generate_function_binding (String.strip (Array.get fn 2)) processed_arguments (String.strip (Array.get fn 1)) in
        Printf.fprintf out_file "%s\n" generated_fn;
        ) with InvalidFunction x -> printf "Could not synthesize bindings for %s because %s\n" (Array.get fn 1) x;
        ()
      )
      | _ -> ()
      );
...
process_header_file "/home/blake/libgpiod_ocaml/src/ml_gpiod_prelude.c" "/home/blake/libgpiod_ocaml/libgpiod/include/gpiod.h" "/home/blake/libgpiod_ocaml/src/ml_gpiod.c";;
